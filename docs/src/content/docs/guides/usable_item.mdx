...
:::tip
This guide will not go into detail on how to create assets, but for creating block models, we recommend [Blockbench](https://www.blockbench.net/) and [Audacity](https://www.audacityteam.org/) for sound effects.
:::
...
3. Open the `pack/assets/minecraft/models/item/clay_ball.json` file and add an override for your model.
    For more information see check out the [Minecraft Wiki](https://minecraft.wiki/w/Items_model_definition).
4. Add your sound files to the `pack/assets/kanoho/sounds/item` directory, and configure them in the `pack/assets/kanoho/sounds.json` file.
    For more information on the configuration options, check out the [Minecraft Wiki](https://minecraft.wiki/w/Sounds.json).
...
Now that you have your assets ready, you can create the item itself. This involves adding the right [data components](https://minecraft.wiki/w/Data_component_format) 
and [Kanoho components](../../reference/item_components) to your give command.
...
2. Add a cooldown to the item by adding the `minecraft:use_cooldown` component.
    ```mcfunction
    minecraft:use_cooldown={seconds:1,cooldown_group:"doc_party_popper"},
    ```
...
To begin with, you need to create a function that will be called when the item is used.
This function will spawn the confetti at the player's location and play the sound you added earlier.
